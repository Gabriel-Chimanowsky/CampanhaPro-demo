...
class TranscribeMp4KnowledgeSourceJob implements ShouldQueue
{
    ...
    public function handle(
        ...
        if (! is_string($path) || $path === '') {
            $manager->markFailed($document, 'O arquivo MP4 temporario nao esta mais disponivel.');
        ...
        try {
            $result = $transcriber->transcribeMp4(Storage::disk($disk)->path($path), ['title' => $document->title]);
